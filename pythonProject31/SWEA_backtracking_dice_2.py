...
path = [0] * 10  # 주사위를 던저 나온 값을 저장하는 path 리스트
def printpath(level):
    print(' '.join(map(str, path[:level])))  # 주사위 값을 문자열로 변환하여 공백을 넣어 출력

def roll1(level):   # M = 1 일때 N 번 주사위 던져서 나올 수 있는 모든 경우
    if level == N: #주사위를 N 번 던졌다면 결과출력
        printpath(level)
        return
...
def roll2(level):    # M = 2 일 때
    if level == N:   # 주사위를 N 번 던졌다면 결과 출력
        printpath(level)
        return
    for i in range(1, 7):
